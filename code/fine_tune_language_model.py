...
def load_train_test_data(train_file_path, validation_file_path, tokenizer):
    '''
    Function to load training and test data. The input format is one tweet per line:
    tweet_id \t tweet text \t OFF/NOT
    :param train_file_path: the path to the training file
    :param validation_file_path: the path to the validation file
    ...
    # -----------------------------------------------------------------
    # Parse Training Set
    with open(train_file_path) as input_file:
        # For each tweet
        count = 0
    ...
    # -----------------------------------------------------------------
    # Parse Test Set
    with open(validation_file_path) as input_file:
        # For each tweet
        for line in csv.reader(input_file, delimiter="\t"):
            if line[0] != 'id' and len(line) == 3:
                full_line = line[1]
...
if len(sys.argv) < 5:
    ...
    print("Usage:")
    print("python fine_tune_language_model.py /path/to/pre-trained/model /path/to/output/dir /path/to/input/training/file /path/to/input/validation/file")
    print("Example:")
    print("python fine_tune_language_model.py ../data/en/a/pretrained_lm ../data/en/a/finetuned_text_classification ../data/en/a/fine-tuning/fine-tuning_training_data.tab ../data/en/a/fine-tuning/fine-tuning_validation_data.tab")
...
# Input training file
input_training_file_path = sys.argv[3]

# Input training file
input_validation_file_path = sys.argv[4]
...
train_inputs, train_labels, train_masks, validation_inputs, validation_labels, validation_masks = load_train_test_data(input_training_file_path, input_validation_file_path, tokenizer)
# train_inputs, train_labels, train_masks = load_train_data(language, tokenizer)
# --------------------------------------------------------------------
...
for epoch_i in tqdm(range(0, epochs), desc="Training"):
    ...
    for batch in validation_dataloader:
        ...
        logger.info(classification_report(pred_flat, labels_flat))

        global_pred.extend(pred_flat)
        global_labels.extend(labels_flat)
...
logger.info("---------- Full evaluation numbers ----------")
logger.info(classification_report(global_pred, global_labels))
